...
export default function Account() {
  ...
  const [activeSubTab, setActiveSubTab] = useState('settings'); // 'settings', 'staff', 'activity'
  const [staffActivityLog, setStaffActivityLog] = useState([]);
  ...
  useEffect(() => {
    if (activeSubTab === 'activity') {
      const loadStaffActivity = async () => {
        ...
        try {
          const res = await api.get('/owner/staff-activity');
          setStaffActivityLog(res.data?.data || []);
        } catch (err) {
          console.error('Failed to load staff activity log:', err);
        } finally {
  ...
  const handleAddStaff = async (e) => {
    ...
    if (newStaff.propertyIds.length === 0) {
    ...
    try {
      const payload = {
        ...
        propertyIds: newStaff.propertyIds.map(Number),
      };
    ...
    <div className="p-4 md:p-8 bg-slate-50 min-h-full max-w-6xl mx-auto space-y-8">
      ...
      {activeSubTab === 'settings' && (
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          {/* Right Section: Quick Expenses Setup */}
      ...
      {activeSubTab === 'staff' && (
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 space-y-6 max-w-4xl mx-auto">
          <div className="flex items-center justify-between border-b border-slate-50 pb-4">
            <h2 className="text-base font-black text-slate-800 uppercase tracking-widest flex items-center gap-3">
                  ...
                  <tr key={staff.id} className="group">
                    ...
                    <td className="py-4 text-center">
                      <button 
                        onClick={async () => {
                          try {
                            const nextStatus = !staff.disabled ? 'blocked' : 'active';
                            await api.patch(`/admin/users/${staff.id}/status`, { status: nextStatus });
                            setStaffList(staffList.map(s => s.id === staff.id ? {...s, disabled: !s.disabled} : s));
                            showToast(`Staff status updated to ${nextStatus}`, 'success');
                          } catch (err) {
        ...
        <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 space-y-6 max-w-4xl mx-auto">
          ...
          {isLoadingActivity ? (
            <div className="text-center py-8">
              ...
              <p className="text-xs text-slate-400 mt-2 font-bold uppercase tracking-widest animate-pulse">Loading Logs...</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-slate-50">
                    <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Staff Member</th>
                    <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Action</th>
                    <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest">Details</th>
                    <th className="pb-4 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Time</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {staffActivityLog.map((log) => (
                    <tr key={log.id} className="group">
                      <td className="py-4">
                        <p className="text-xs font-black text-slate-800">{log.staffFirstName} {log.staffLastName}</p>
                        <p className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter">@{log.staffUsername}</p>
                      </td>
                      <td className="py-4">
                      ...
                      <td className="py-4 text-xs font-bold text-slate-600 max-w-xs truncate" title={log.details}>
                        {log.details}
                      </td>
                  ...
                  {!staffActivityLog.length && (
                    <tr>
                      <td colSpan="4" className="py-8 text-center text-[10px] font-black text-slate-400 uppercase tracking-widest italic">
                        No Activity Logged
                      </td>
                    </tr>
                  )}
      ...
      {showAddStaff && (
        <div className="fixed inset-0 z-[100] bg-slate-900/40 backdrop-blur-md flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white rounded-3xl p-8 shadow-2xl animate-in fade-in zoom-in duration-300 max-h-[90vh] overflow-y-auto custom-scrollbar">
            ...
            <form onSubmit={handleAddStaff} className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Assigned Properties</label>
                <div className="grid grid-cols-2 gap-2">
                  {propertyOptions.map((property) => {
                    const isSelected = newStaff.propertyIds.includes(property.id);
                    return (
                      <button
                        key={property.id}
                        type="button"
                      ...
                      >
                        ...
                        {isSelected ? (
                          <svg className="w-4 h-4 text-white shrink-0 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>
                        ) : (
                          <div className="w-4 h-4 rounded-md border border-slate-300 shrink-0 ml-2" />
                        )}
                      </button>
                    );
                  })}
                  {!propertyOptions.length && (
                    <p className="text-xs text-slate-400 italic col-span-2">No properties found</p>
                  )}
                </div>
              </div>
